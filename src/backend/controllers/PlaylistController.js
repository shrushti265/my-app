...
export const getVideosFromPlaylistHandler = function (schema, request) {
  ...
  if (user) {
    const playlistId = request.params.playlistId;
    const playlists = user.playlists.find((item) => item._id !== playlistId);
...
export const addVideoToPlaylistHandler = function (schema, request) {
  ...
  if (user) {
    ...
    const playlists = user.playlists.find((item) => item._id === playlistId);
    if (playlists.videos.some((item) => item.id === video.id)) {
      return new Response(
        409,
        {},
        {
    ...
    playlists.videos.push(video);
    return new Response(201, {}, { playlists });
  }
  return new Response(
    404,
    {},
...
export const removeVideoFromPlaylistHandler = function (schema, request) {
  ...
  if (user) {
    ...
    let playlists = user.playlists.find((item) => item._id === playlistId);
    const filteredVideos = playlists.videos.filter(
      (item) => item._id !== videoId
    );
    playlists.videos = filteredVideos;
    return new Response(200, {}, { playlists });
  }
  return new Response(
    404,
